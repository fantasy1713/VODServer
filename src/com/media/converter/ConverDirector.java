...
/*	public static void main(String []args){
		ConverDirector dir = new ConverDirector();
	//	for(int i=0; i<10; i++)
	//		dir.test("6.ts", "d:\\3", counter++ + ".mp4", "d:\\3\\2", i, true);
		
	//	counter = 100;
	//	for(int i=0; i<10; i++)
	//		dir.test("6.ts", "d:\\3", counter++ + ".mp4", "d:\\3\\1", i, false);
	}
	
	public void test2(String filename, String filepath, String targetname, String targetpath, int plv, Boolean twopass){
		File test = new File("d:\\time\\" + filename + "_" + plv + "_" + twopass.toString() + ".txt");
		FileWriter fw = null;
		long start = System.currentTimeMillis();
		try {
			fw = new FileWriter(test, true);
			fw.write(filename + "_" + plv);
		...
		BaseConverter dashcon = (DashConverter) dashfact.createConverter(targetpath, targetname,
				targetpath + "/dash/" + targetname, targetname + "." + dashfact.getFileSuffix());
		...
		try {
			fw.write("\r\ndashfinished at:" + String.valueOf(System.currentTimeMillis() - start));
			fw.flush();
			fw.close();
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void test(String filename, String filepath, String targetname, String targetpath, int plv, Boolean twopass){
		File test = new File("d:\\time\\" + filename + "_" + plv + "_" + twopass.toString() + ".txt");
		FileWriter fw = null;
		long start = System.currentTimeMillis();
		try {
			fw = new FileWriter(test, true);
			fw.write(filename + "_" + plv + " start at ");
			SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		...
		BaseConverter mp4con = (Mp4Converter) mp4fact.createConverter(filepath, filename, targetpath, targetname);
		mp4con.setM_presetlv(plv);
		//将媒体信息对象作为参数传入方法，并通过返回的对象来更新自己
		mp4con.startConver();
		
		try {
			fw.write("\r\nmp4finished at:" + String.valueOf(System.currentTimeMillis() - start));
			SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			fw.write("\r\n" + format.format(new Date()));
	...
	public void startConverter(MediaPipelineTaskClass task, List<Mediafileinfo> list){
		Session sen = HibernateSessionFactory.getSession();
		Query query = sen.createQuery("from Mediafileprototype where id=:id").setInteger("id", task.getPrototypeid());
		Mediafileprototype proinfo = (Mediafileprototype) query.uniqueResult();

		Mp4ConverterFactory mp4fact = new Mp4ConverterFactory();
		BaseConverter mp4con = (Mp4Converter) mp4fact.createConverter(task.getFilepath(), task.getFilename(),
					mp4fact.getFileStorePath(), task.getFilename() + "." + mp4fact.getFileSuffix());
		Mediafileinfo mp4info = this.getMediaInfoByType(mp4fact.getFileType(), list);
		//将媒体信息对象作为参数传入方法，并通过返回的对象来更新自己
		mp4info = this.converOperate(mp4info, mp4con, proinfo);
		
		//dash是基于mp4视频源进行的编码
		if(mp4info != null && mp4info.getStatus() == MediaFileStatusDefine.READY){
			DashConverterFactory dashfact = new DashConverterFactory();
			String filename = this.getFilename(mp4info.getFilename());
			BaseConverter dashcon = (DashConverter) dashfact.createConverter(mp4con.getOutPath(), mp4con.getOutName(),
					dashfact.getFileStorePath() + "/" + filename, filename + "." + dashfact.getFileSuffix());
			...
			Mediafileinfo dashinfo = this.getMediaInfoByType(dashfact.getFileType(), list);
			//将媒体信息对象作为参数传入方法，并通过返回的对象来更新自己
			dashinfo = this.converOperate(dashinfo, dashcon, proinfo);
		}
	...
	private Mediafileinfo converOperate(Mediafileinfo mediainfo, BaseConverter con, Mediafileprototype proinfo){
		if(mediainfo == null){
	...
	private Mediafileinfo getMediaInfoByType(String type, List<Mediafileinfo> infolist){
