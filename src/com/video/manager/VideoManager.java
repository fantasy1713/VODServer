...
public class VideoManager {
	...
	public String findMediaExists(List<WeblibUrlInfo> urllist){
		...
		try{
			...
			for(int i=0; i<urllist.size(); i++){
				if(i != 0)
					sb.append("or ");
				sb.append("(weblibhost='" + urllist.get(i).url + "' and weblibid=" + urllist.get(i).id + " and viewindex=" + urllist.get(i).viewindex + ") ");
			}
	...
	public String createMediaQuery(List<WeblibUrlInfo> urllist){
		...
		try
		{
			...
			for(int i=0; i<urllist.size(); i++){
				if(i != 0)
					sb.append("or ");
				sb.append("(weblibhost='" + urllist.get(i).url + "' and weblibid=" + urllist.get(i).id + ") ");
			}
			Query query = sen.createSQLQuery(sb.toString());
			List list = query.list();
			...
			for(WeblibUrlInfo info : urllist){
				...
				for(ExistCacheState item : extlist){
					if(item.weblibhost.equalsIgnoreCase(info.url) && item.weblibid == info.id){
						Query extquery = sen.createQuery("from Mediafileprototype where id=" + item.prototypeid);
						info.prototype = (Mediafileprototype) extquery.uniqueResult();
						found = true;
						break;
					}
				}
			...
			do
			{
				...
				String idquery = "select count(1) from vodmedia where urlstring='" + vodid + "'";
			...
			//创建媒体记录信息
			Vodmedia media = new Vodmedia(null, vodid, viewlist.size(), null, null, "preparing", null, null, new Timestamp(new Date().getTime())
			, 0, null, 0, null, null, new Timestamp(new Date().getTime()), 0, null);
			sen.save(media);
			...
			for(WeblibUrlInfo info : urllist){
				...
				Vodmediaparams param = new Vodmediaparams(paramid, info.prototype, info.url, info.id, null, info.orignalurl, info.startoffset);
				sen.save(param);
			}
	...
	public void updateMediaContent(String mediastring){
		...
		String str = "select t1.id as mid, t2.orderindex, t2.viewindex, t2.weblibhost, t2.weblibid, t3.id, t4.status from vodmedia t1 " +
				...
				"where t1.urlstring='" + mediastring + "' order by t2.orderindex";
		Session sen = HibernateSessionFactory.getSession();
		...
		for(MediaFileState fileinfo : filelist){
			if(fileinfo.status == null
				...
				boolean result = MediaPipelineBuilder.addNewQuest(
						fileinfo.weblibhost,
						fileinfo.weblibid,
						fileinfo.prototypeid,
						ServerSettings.getDownloadPath(),
	...
	public void refreshMediaStatus(int mediaid, String status){
		Session sen = HibernateSessionFactory.getSession();
		Transaction tran = sen.beginTransaction();
		Query query = sen.createQuery("Update Vodmedia set status=:status where id=:id");
	...
	public JsonResponseInterface getMediaContents(String mediaid){
		...
		Session sen = HibernateSessionFactory.getSession();
		Query query1 = sen.createQuery("from Vodmedia where urlstring=:urlstring");
		...
		Query query2 = sen.createSQLQuery("select t2.id, t1.viewindex, t1.orderindex, t2.filetype," +
				...
				" mediafileinfo t2 where t1.srcfileid=t2.srcfileid and t1.mediaid=" + media.getId() +
		...
		for(int i=0; i<list.size(); i++){
			...
			if(orderinfo == null || orderinfo.getOrderindex() != orderindex){
				...
				orderinfo.setSegments(new ArrayList<MediaResponseSegmentJson>());
			...
			seginfo.setHeight(itemquery[10] != null ? (Integer)itemquery[10] : 0);
	...
	public String getAttrCode(String id) {
		Session sess  =HibernateSessionFactory.getSession();
		Mediafileinfo mf = (Mediafileinfo) sess.get(Mediafileinfo.class, Integer.parseInt(id));
